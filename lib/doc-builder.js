...
async function buildContractDocx({ contract, vehicle, renterName, termsText }) {
  ...
  const children = [
    ...
    kvTable([
      ...
      ['Наемна цена', `${contract.rate_amount} лв. / ${RATE_PERIOD_LABELS[contract.rate_period] || contract.rate_period}`],
      ['Депозит', `${contract.deposit_amount || 0} лв.`],
...
async function buildEmploymentContractDocx({ contract, profile, termsText }) {
  ...
  children.push(
    ...
    kvTable([
      ...
      ['Начална дата', contract.start_date],
      ['Крайна дата', contract.end_date || 'безсрочен'],
      ['Седмична удръжка', `${contract.weekly_deduction_amount || 0} лв.`],
    ]),
  );
