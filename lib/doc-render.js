...
function flattenEmploymentContract(contract, profile) {
  return {
    ...
    weekly_deduction_amount: `${contract.weekly_deduction_amount || 0} лв.`,
...
function flattenContract(contract, vehicle, renterName) {
  return {
    ...
    start_date: contract.start_date,
    end_date: contract.end_date || 'безсрочен',
    rate_amount: `${contract.rate_amount} лв. / ${RATE_PERIOD_LABELS[contract.rate_period] || contract.rate_period}`,
    deposit_amount: `${contract.deposit_amount || 0} лв.`,
