...
function updateUser(id, patch) {
  ...
  // имейлът е уникалният идентификатор за вход (виж findUserByEmail/login) —
  // пазим същата защита срещу дублиране като при createUser, но изключваме
  // текущия профил, за да може админ да запази без промяна на имейла
  if (patch.email && db.profiles.some(p => p.id !== id && p.email.toLowerCase() === String(patch.email).toLowerCase())) {
    throw new Error('Вече има потребител с този имейл');
  }
...
// ---------------------------------------------------------------------------
// СЪДЪРЖАНИЕ НА НОВИЯ РЕНТ-А-КАР САЙТ (rent-a-car-site/index.html, отделен
// от dombi.bg) — GET/PUT /api/rentacar-site-content. GET е публичен (без
// сесия, CORS) — чете го публичният сайт. PUT е само за админ/мениджър,
// вика се от /rentacar-site-editor.html. Списъкът с коли под наем се
// управлява отделно, в „Витрина на сайта“ (fleet-showcase.html) — тук са
// само текстовете (hero, стъпки, условия, отзиви, въпроси, футър).
// ---------------------------------------------------------------------------
const DEFAULT_RENTACAR_CONTENT = {
  hero: {
    eyebrow: 'Dombi Rent-a-Car · Автопарк под наем',
    title_line1: 'Наеми кола',
    title_line2: 'за минути,',
    title_line3: 'не за дни.',
    lead: 'Избери автомобил, посочи дати и изпрати заявка — потвърждаваме бързо по телефон. Застраховка и пътна помощ са включени във всеки наем, без скрити такси.',
    cta_primary: 'Провери наличност →',
    cta_secondary: 'Разгледай автопарка',
    trust_items: ['Застраховка включена', '24/7 пътна помощ', 'Без скрити такси'],
    phone_display: '0887 25 27 27',
    phone_tel: '+359887252727',
  },
  fleet_section: {
    eyebrow: 'Автопарк',
    title: 'Избери своя автомобил',
    subtitle: 'Всички коли се предлагат със застраховка и пътна помощ. Наличността по-долу е за избрания от теб период — или разгледай пълния автопарк.',
  },
  how: {
    eyebrow: 'Как работи',
    title: 'От избор до волана — 4 стъпки',
    steps: [
      { title: 'Избери кола и дати', desc: 'Прегледай автопарка и провери жива наличност за периода, който ти трябва.' },
      { title: 'Изпрати заявка', desc: 'Попълваш кратка форма с данни за контакт — без плащане на този етап.' },
      { title: 'Потвърждаваме по телефон', desc: 'Екипът ни се свързва с теб до няколко часа, за да потвърди резервацията.' },
      { title: 'Взимаш колата', desc: 'Подписваме кратък договор на място и потегляш — просто и бързо.' },
    ],
  },
  policy: {
    eyebrow: 'Условия',
    title: 'Прозрачни условия, без изненади',
    subtitle: 'Всичко, което трябва да знаеш преди да изпратиш заявка.',
    included_title: 'Включено в наема',
    included: ['Застраховка "Гражданска отговорност"', 'Пътна помощ 24/7', 'Годишен преглед и поддръжка', 'Смяна на автомобил при повреда не по твоя вина'],
    requirements_title: 'Изисквания за наемане',
    requirements: ['Навършени 21 години', 'Свидетелство за управление с минимум 1 година стаж', 'Лична карта или паспорт', 'Депозит при взимане на автомобила'],
    flexibility_title: 'Гъвкавост',
    flexibility: ['Заявката е безплатна и необвързваща', 'Без онлайн плащане при резервация', 'Удължаване на наема при наличност', 'Отказ по всяко време преди потвърждение'],
  },
  stats: {
    response_time: '< 2ч',
    hidden_fees: '0 лв',
  ...
  testimonials: {
    eyebrow: 'Отзиви',
    title: 'Какво казват наши клиенти',
    items: [
      { quote: 'Изпратих заявка вечерта и още на сутринта ме потвърдиха по телефона. Колата беше чиста и в отлично състояние.', name: 'Клиент', sub: 'Наем за 5 дни · София' },
      { quote: 'Трябваше ми кола спешно за уикенда — целият процес отне по-малко от час, включително договора на място.', name: 'Клиент', sub: 'Уикенд наем · Пловдив' },
      { quote: 'Оценявам, че няма скрити такси — цената, която видях онлайн, съвпадна напълно с крайната сметка.', name: 'Клиент', sub: 'Месечен наем · Варна' },
    ],
    note: 'Примерни отзиви — ще бъдат заменени с реални мнения на клиенти.',
  },
  faq: {
    eyebrow: 'Въпроси и отговори',
    title: 'Често задавани въпроси',
    items: [
      { q: 'Как протича резервацията?', a: 'Избираш кола и период, изпращаш заявка от сайта — без онлайн плащане. Екипът ни се свързва с теб по телефон, за да потвърди резервацията и да уговори детайлите.' },
      { q: 'Какви документи са нужни за наемане?', a: 'Лична карта или паспорт и валидно свидетелство за управление с минимум 1 година стаж. Изисква се навършени 21 години.' },
      { q: 'Има ли депозит?', a: 'Да, при взимане на автомобила се удържа депозит, който се възстановява при връщането му в изправно състояние.' },
      { q: 'Какво е включено в цената?', a: 'Застраховка "Гражданска отговорност" и пътна помощ 24/7 са включени в наема. Гориво не е включено — автомобилът се взима и връща с равнозначно количество гориво.' },
      { q: 'Мога ли да удължа наема?', a: 'Да, свържи се с нас преди изтичане на текущия период — удължаваме при наличност на автомобила.' },
      { q: 'Мога ли да откажа заявка?', a: 'Разбира се — заявката е необвързваща до момента на потвърждение от наша страна. Можеш да откажеш по всяко време преди това, като ни се обадиш.' },
    ],
  },
  cta_band: {
    title: 'Готов ли си да потеглиш?',
    subtitle: 'Провери жива наличност за твоите дати и изпрати заявка за секунди.',
  },
  footer: {
    tagline: 'Част от Dombi Riders ЕООД — автопарк за наем с прозрачни условия, застраховка и пътна помощ, включени във всеки наем.',
    phone_display: '0887 25 27 27',
    phone_tel: '+359887252727',
    email: 'office@dombi.bg',
    address: 'гр. София, ул. Павел Красов 22',
  },
};

function getRentacarSiteContent() {
  const db = readDb();
  const saved = db.rentacar_site_content || {};
  const merged = {};
  Object.keys(DEFAULT_RENTACAR_CONTENT).forEach(section => {
    merged[section] = { ...DEFAULT_RENTACAR_CONTENT[section], ...(saved[section] || {}) };
  });
  return merged;
}

// вариант на scStrArrayFixed за списъци с ПРОМЕНЛИВА дължина (напр. условия,
// въведени по един ред в текстово поле) — пази до maxItems непразни низа,
// при невалиден/празен вход пада обратно към текущата стойност.
function rcStrArrayVar(v, d, maxItems, maxLen = 300) {
  if (!Array.isArray(v)) return d;
  const out = v.filter(x => typeof x === 'string').map(x => x.slice(0, maxLen)).filter(x => x.trim()).slice(0, maxItems);
  return out.length ? out : d;
}
// (без фолбек към default при празен резултат — админът може съзнателно
// да изтрие всички отзиви/въпроси, точно както в scTestimonials по-горе.)
function rcTestimonials(v, d) {
  if (!Array.isArray(v)) return d;
  return v.slice(0, 12).map(item => ({
    quote: typeof (item || {}).quote === 'string' ? item.quote.slice(0, 600) : '',
    name: typeof (item || {}).name === 'string' && item.name.trim() ? item.name.slice(0, 100) : 'Клиент',
    sub: typeof (item || {}).sub === 'string' ? item.sub.slice(0, 150) : '',
  })).filter(t => t.quote.trim());
}
function rcQaArray(v, d, maxItems = 20) {
  if (!Array.isArray(v)) return d;
  return v.map(item => ({
    q: typeof (item || {}).q === 'string' ? item.q.slice(0, 300) : '',
    a: typeof (item || {}).a === 'string' ? item.a.slice(0, 1000) : '',
  })).filter(x => x.q.trim() && x.a.trim()).slice(0, maxItems);
}

// PUT приема ЦЯЛАТА структура (виж коментара при updateSiteContent по-горе
// за причината) — всяко поле се проверява по тип/дължина спрямо
// DEFAULT_RENTACAR_CONTENT (whitelist), непознати ключове се игнорират.
function updateRentacarSiteContent(patch) {
  const current = getRentacarSiteContent();
  const p = patch && typeof patch === 'object' ? patch : {};
  const hero = p.hero || {}, fleetSection = p.fleet_section || {}, how = p.how || {}, policy = p.policy || {}, stats = p.stats || {},
    testimonials = p.testimonials || {}, faq = p.faq || {}, ctaBand = p.cta_band || {}, footer = p.footer || {};

  const next = {
    hero: {
      eyebrow: scStr(hero.eyebrow, current.hero.eyebrow, 150),
      title_line1: scStr(hero.title_line1, current.hero.title_line1, 60),
      title_line2: scStr(hero.title_line2, current.hero.title_line2, 60),
      title_line3: scStr(hero.title_line3, current.hero.title_line3, 60),
      lead: scStr(hero.lead, current.hero.lead, 500),
      cta_primary: scStr(hero.cta_primary, current.hero.cta_primary, 60),
      cta_secondary: scStr(hero.cta_secondary, current.hero.cta_secondary, 60),
      trust_items: rcStrArrayVar(hero.trust_items, current.hero.trust_items, 4, 80),
      phone_display: scStr(hero.phone_display, current.hero.phone_display, 40),
      phone_tel: scStr(hero.phone_tel, current.hero.phone_tel, 40),
    },
    fleet_section: {
      eyebrow: scStr(fleetSection.eyebrow, current.fleet_section.eyebrow, 120),
      title: scStr(fleetSection.title, current.fleet_section.title, 200),
      subtitle: scStr(fleetSection.subtitle, current.fleet_section.subtitle, 400),
    },
    how: {
      eyebrow: scStr(how.eyebrow, current.how.eyebrow, 120),
      title: scStr(how.title, current.how.title, 200),
    ...
    policy: {
      eyebrow: scStr(policy.eyebrow, current.policy.eyebrow, 120),
      title: scStr(policy.title, current.policy.title, 200),
      subtitle: scStr(policy.subtitle, current.policy.subtitle, 400),
      included_title: scStr(policy.included_title, current.policy.included_title, 100),
      included: rcStrArrayVar(policy.included, current.policy.included, 8, 200),
      requirements_title: scStr(policy.requirements_title, current.policy.requirements_title, 100),
      requirements: rcStrArrayVar(policy.requirements, current.policy.requirements, 8, 200),
      flexibility_title: scStr(policy.flexibility_title, current.policy.flexibility_title, 100),
      flexibility: rcStrArrayVar(policy.flexibility, current.policy.flexibility, 8, 200),
    },
    stats: {
      response_time: scStr(stats.response_time, current.stats.response_time, 20),
      hidden_fees: scStr(stats.hidden_fees, current.stats.hidden_fees, 20),
    },
    testimonials: {
      eyebrow: scStr(testimonials.eyebrow, current.testimonials.eyebrow, 120),
      title: scStr(testimonials.title, current.testimonials.title, 200),
      items: rcTestimonials(testimonials.items, current.testimonials.items),
      note: scStr(testimonials.note, current.testimonials.note, 300),
    },
    faq: {
      eyebrow: scStr(faq.eyebrow, current.faq.eyebrow, 120),
      title: scStr(faq.title, current.faq.title, 200),
      items: rcQaArray(faq.items, current.faq.items, 20),
    },
    cta_band: {
      title: scStr(ctaBand.title, current.cta_band.title, 200),
      subtitle: scStr(ctaBand.subtitle, current.cta_band.subtitle, 300),
    },
    footer: {
      tagline: scStr(footer.tagline, current.footer.tagline, 400),
      phone_display: scStr(footer.phone_display, current.footer.phone_display, 40),
      phone_tel: scStr(footer.phone_tel, current.footer.phone_tel, 40),
      email: scStr(footer.email, current.footer.email, 200),
      address: scStr(footer.address, current.footer.address, 300),
    },
  };
  const db = readDb();
  db.rentacar_site_content = next;
  writeDb(db);
  return next;
}
...
function listContracts({ vehicleId } = {}) {
  ...
  return rows.map(c => ({
    ...c,
    balance: computeContractBalance(c, db.vehicle_payments),
    deposit: getContractDepositState(c.id, db.wallet_transactions),
  }));
}

// ---------------------------------------------------------------------------
// ДЕПОЗИТ ПО ДОГОВОР ЗА НАЕМ — отделно от наемния баланс (computeContractBalance
// по-горе). deposit_amount на самия договор е само ДОГОВОРЕНАТА сума; дали
// депозитът РЕАЛНО е взет/върнат в брой се пази като собствени касови
// транзакции (deposit_received/deposit_returned в wallet_transactions,
// свързани чрез related_contract_id) — по същия "изведи на живо от
// транзакциите" принцип като computeContractBalance, вместо флагове на
// самия договор, за да поддържа и частично връщане (напр. удържане за щета)
// и пълна история кога какво е станало.
// ---------------------------------------------------------------------------
function getContractDepositState(contractId, allWalletTx) {
  const txs = (allWalletTx || readDb().wallet_transactions || [])
    .filter(t => t.related_contract_id === contractId && (t.type === 'deposit_received' || t.type === 'deposit_returned'))
    .sort((a, b) => (a.created_at < b.created_at ? -1 : 1));
  const takenTx = txs.filter(t => t.type === 'deposit_received');
  const returnedTx = txs.filter(t => t.type === 'deposit_returned');
  const takenAmount = takenTx.reduce((s, t) => s + Number(t.amount || 0), 0);
  const returnedAmount = returnedTx.reduce((s, t) => s + Math.abs(Number(t.amount || 0)), 0);
  return {
    taken: takenTx.length > 0,
    taken_amount: Math.round(takenAmount * 100) / 100,
    taken_at: takenTx[0]?.created_at || null,
    returned: returnedTx.length > 0,
    returned_amount: Math.round(returnedAmount * 100) / 100,
    returned_at: returnedTx.length ? returnedTx[returnedTx.length - 1].created_at : null,
    currently_held: Math.round((takenAmount - returnedAmount) * 100) / 100,
    history: txs,
  };
}

// записва РЕАЛНО взет депозит в брой по договор — влиза директно в касата
// (реални пари в брой), но НЕ се брои в приход/печалба, виж
// CASHIER_TX_TYPES_EXCLUDED_FROM_PROFIT и getCashierPeriodSummary.
function addContractDeposit({ contractId, amount, note, createdBy }) {
  const db = readDb();
  const contract = db.rental_contracts.find(c => c.id === contractId);
  if (!contract) throw new Error('Договорът не е намерен');
  const cashierId = db.hr_settings?.cashier_profile_id;
  if (!cashierId) throw new Error('Касата не е настроена — задайте титуляр на касата в Настройки.');
  const amt = Number(amount);
  if (!amt || amt <= 0) throw new Error('Сумата на депозита трябва да е положително число.');
  if (!db.wallet_transactions) db.wallet_transactions = [];
  db.wallet_transactions.push({
    id: uid('wtx'), user_id: cashierId, amount: amt,
    type: 'deposit_received', related_contract_id: contractId,
    note: note || `Взет депозит — договор ${contract.contract_number}${contract.renter_name ? ' · ' + contract.renter_name : ''}`,
    created_by: createdBy || null, created_at: nowIso(),
  });
  writeDb(db);
  return { deposit: getContractDepositState(contractId, db.wallet_transactions), cashier_balance: getCashierBalance(cashierId) };
}

// записва РЕАЛНО върнат депозит (изцяло или частично — напр. удържане за
// щета по колата, обяснено в note) — излиза от касата, но пак НЕ се брои в
// приход/разход/печалба (виж по-горе).
function returnContractDeposit({ contractId, amount, note, createdBy }) {
  const db = readDb();
  const contract = db.rental_contracts.find(c => c.id === contractId);
  if (!contract) throw new Error('Договорът не е намерен');
  const cashierId = db.hr_settings?.cashier_profile_id;
  if (!cashierId) throw new Error('Касата не е настроена — задайте титуляр на касата в Настройки.');
  const amt = Number(amount);
  if (!amt || amt <= 0) throw new Error('Сумата за връщане трябва да е положително число.');
  const state = getContractDepositState(contractId, db.wallet_transactions);
  if (!state.taken) throw new Error('За този договор все още не е записано вземане на депозит.');
  if (amt > state.currently_held + 0.01) {
    throw new Error(`Не може да върнете повече от задържания в момента депозит (${state.currently_held.toFixed(2)} €).`);
  }
  if (!db.wallet_transactions) db.wallet_transactions = [];
  db.wallet_transactions.push({
    id: uid('wtx'), user_id: cashierId, amount: -amt,
    type: 'deposit_returned', related_contract_id: contractId,
    note: note || `Върнат депозит — договор ${contract.contract_number}${contract.renter_name ? ' · ' + contract.renter_name : ''}`,
    created_by: createdBy || null, created_at: nowIso(),
  });
  writeDb(db);
  return { deposit: getContractDepositState(contractId, db.wallet_transactions), cashier_balance: getCashierBalance(cashierId) };
...
// сума на ВСИЧКИ в момента задържани депозити (всички договори, за да се
// вижда като статистика в касата — виж getCashierPeriodSummary) — не е
// ограничена по период, защото е "текущо състояние", не поток за периода.
function getTotalDepositsHeld() {
  const db = readDb();
  const byContract = {};
  (db.wallet_transactions || []).forEach(t => {
    if (t.type !== 'deposit_received' && t.type !== 'deposit_returned') return;
    if (!t.related_contract_id) return;
    byContract[t.related_contract_id] = (byContract[t.related_contract_id] || 0) + Number(t.amount || 0);
  });
  return Math.round(Object.values(byContract).reduce((s, v) => s + Math.max(0, v), 0) * 100) / 100;
}
...
function getContract(id) {
  ...
  if (!c) return null;
  return { ...c, balance: computeContractBalance(c, db.vehicle_payments), deposit: getContractDepositState(c.id, db.wallet_transactions) };
}
...
function addPayment(data) {
  ...
  writeDb(db);
  // плащане по договор с ВЪНШЕН наемател (не куриер на Dombi Riders) е
  // реални пари в брой, платени директно на касиера — за куриерите наемът
  // вече е удържан от заработката (viж upsertPayrollEntry/getNonCashPayrollStats),
  // затова само външните договори се синхронизират тук, за да няма двойно броене
  if (rec.contract_id) {
    const contract = db.rental_contracts.find(c => c.id === rec.contract_id);
    if (contract && contract.renter_type !== 'dombi_courier') {
      syncExternalRentCashierEntry(rec, contract);
    }
  }
  return rec;
}
...
// такса на фирмата, докато наемът на колата е приход, който трябва да се
// вижда като приход на КОНКРЕТНАТА кола в статистиката (виж getFleetStats).
// ⚠️ ВАЖНО (променено по изрично изискване на потребителя): наемът на кола и
// удръжката по договор НЕ постъпват автоматично в общата каса — те са само
// СТАТИСТИКА (виж getNonCashPayrollStats), защото представляват пари, които
// никога не са напускали "джоба" на шофьора в брой — просто са приспаднати
// от това, което ще му се изплати. Реалното касово движение става едва при
// ФАКТИЧЕСКОТО изплащане на нетната сума (виж markPayrollPaid/
// syncWagePayoutKasaEntry по-долу).
// изчислява default удръжка по договор / наем на кола за служител, когато
// не са зададени изрично — огледално на логиката в payroll.html за РЪЧНО
// въвеждане на седмица, но приложено и при АВТОМАТИЧНИЯ импорт от Bolt/Glovo
// (виж /api/hr/payroll/import/apply в server.js) — иначе за десетки служители
// никой реално не влиза да ги попълва ръчно всяка седмица и статистиката на
// автопарка/заплатите остава постоянно на 0, въпреки активни договори.
// 'draft' договори (никога финализирани) съзнателно НЕ се броят — вж. и
// getEmployeeDocumentAlerts по-долу за същата логика при тревогите.
const PAYROLL_RATE_PERIOD_DAYS = { day: 1, week: 7, month: 30 };
function getDefaultPayrollDeductions(profileId) {
  const db = readDb();
  const activeContract = (db.employment_contracts || [])
    .filter(c => c.profile_id === profileId && c.status === 'active')
    .sort((a, b) => (a.start_date < b.start_date ? 1 : -1))[0];
  const deductionAmount = activeContract ? Number(activeContract.weekly_deduction_amount || 0) : 0;

  const activeRental = (db.rental_contracts || [])
    .filter(c => c.renter_driver_id === profileId && c.status === 'active')
    .sort((a, b) => (a.start_date < b.start_date ? 1 : -1))[0];
  let carRentAmount = 0;
  if (activeRental) {
    // договорът за наем може да е на ден/седмица/месец — тук винаги
    // удържаме СЕДМИЧНО, затова конвертираме към седмичен еквивалент
    const days = PAYROLL_RATE_PERIOD_DAYS[activeRental.rate_period] || 7;
    carRentAmount = Math.round((Number(activeRental.rate_amount || 0) / days) * 7 * 100) / 100;
  }
  return { deduction_amount: deductionAmount, car_rent_amount: carRentAmount };
}
...
function upsertPayrollEntry(data) {
  ...
  writeDb(db);
  // ако записът вече е маркиран платен и после бъде редактиран (напр. поправка
  // на грешка), опресняваме и вече заведеното касово движение със същата сума
  if (rec.paid) syncWagePayoutKasaEntry(rec);
  return rec;
}

// обобщена (некасова) статистика за наем на коли + удръжки по договор за
// период — именно тези две суми НЕ участват в наличността на касата (виж
// коментара над upsertPayrollEntry по-горе), но трябва да се виждат някъде,
// за да има пълна картина на приходите на фирмата от шофьорите
function getNonCashPayrollStats({ from, to } = {}) {
  const db = readDb();
  let rows = db.payroll_entries || [];
  if (from) rows = rows.filter(e => e.week_start >= from);
  if (to) rows = rows.filter(e => e.week_start <= to);
  const carRentTotal = rows.reduce((s, e) => s + Number(e.car_rent_amount || 0), 0);
  const deductionTotal = rows.reduce((s, e) => s + Number(e.deduction_amount || 0), 0);
  const entries = rows
    .filter(e => e.car_rent_amount || e.deduction_amount)
    .map(e => {
      const driver = findUserById(e.profile_id);
      return {
        profile_id: e.profile_id, driver_name: driver ? driver.full_name : e.profile_id,
        week_start: e.week_start, week_end: e.week_end,
        car_rent_amount: Number(e.car_rent_amount || 0), deduction_amount: Number(e.deduction_amount || 0),
      };
    })
    .sort((a, b) => (a.week_start < b.week_start ? 1 : -1));
  return { from: from || null, to: to || null, car_rent_total: carRentTotal, deduction_total: deductionTotal, total: carRentTotal + deductionTotal, entries };
}
...
// самия портфейл). Тук постъпват само ДЕЙСТВИТЕЛНИ касови движения:
// изплатени заплати, наеми от външни наематели, ремонти, ръчни корекции и
// платени комисионни (виж CASHIER_TX_TYPES по-долу) — НЕ и наемът/удръжките
// по договор, които са чиста статистика (виж getNonCashPayrollStats). Ръчните
// движения по касата остават заключени само за super_admin (виж
// requireSuperAdmin в server.js — нарочно НЕ минава през configurable
// permissions matrix).
// -----------------------------------------------------------------------
...
// ⚠️ КАСАТА НЕ Е ЦЕЛИЯТ ПОРТФЕЙЛ на избрания профил — той може да има и
// СВОИ ЛИЧНИ движения (превод от/към колега, лична корекция от админ) в
// СЪЩАТА таблица wallet_transactions (виж getWalletBalance). Ако вземем
// целия портфейл, лично захранване на касиера си "утежнява" фирмената каса
// без изобщо да са фирмени пари — открит и потвърден бъг при тестване.
// Затова баланс/история на КАСАТА (за разлика от личния портфейл) взимат
// само транзакции с тип, характерен именно за касата — всяка представлява
// РЕАЛНО движение на пари в брой:
//  - 'wage_payout'          — изплатена нетна заработка (виж markPayrollPaid)
//  - 'external_rent_income' — наем, платен в брой от външен наемател (не
//                              куриер на Dombi Riders) по договор за наем
//  - 'external_rent_expense'— друг разход по такъв договор (НЕ депозит — за
//                              депозит виж deposit_received/deposit_returned
//                              по-долу, които се пазят отделно от статистиката
//                              за печалба, защото депозитът не е приход)
//  - 'vehicle_repair'       — ремонт/разход по кола, платен от касата (виж
//                              addCashierRepairEntry — създава и запис в
//                              сервизната книжка)
//  - 'commission_payout'    — изплатена партньорска комисионна
//  - 'cashier_manual'       — друго ръчно движение от super_admin
//  - 'deposit_received'     — взет депозит по договор за наем (виж
//                              addContractDeposit) — реални пари в касата,
//                              но НЕ е приход/печалба (задължение за връщане)
//  - 'deposit_returned'     — върнат депозит по договор (виж
//                              returnContractDeposit) — може да е частичен
//                              (напр. удържано за щета)
// Наемът на кола/удръжката по договор (payroll_entries) НЕ участват тук —
// те са чиста статистика, виж getNonCashPayrollStats. Лични трансфери
// ('transfer') и лични корекции ('admin_adjustment') на същия профил също НЕ
// участват тук — те са си негов личен портфейл (wallet.html).
const CASHIER_TX_TYPES = [
  'wage_payout', 'external_rent_income', 'external_rent_expense',
  'vehicle_repair', 'commission_payout', 'cashier_manual',
  'deposit_received', 'deposit_returned',
];
// типове от горния списък, които СА реални пари в касата, но НЕ бива да
// влизат в изчислението на приход/разход/печалба (getCashierPeriodSummary) —
// депозитът е чуждо задължение, не приход на фирмата, докато не изтече без
// връщане (в който случай някой ще го превключи ръчно през cashier_manual).
const CASHIER_TX_TYPES_EXCLUDED_FROM_PROFIT = new Set(['deposit_received', 'deposit_returned']);

function getCashierBalance(userId) {
  const db = readDb();
  return (db.wallet_transactions || [])
    .filter(t => t.user_id === userId && CASHIER_TX_TYPES.includes(t.type))
    .reduce((sum, t) => sum + Number(t.amount || 0), 0);
}

function listCashierTransactions(userId, { from, to } = {}) {
  const db = readDb();
  let rows = (db.wallet_transactions || [])
    .filter(t => t.user_id === userId && CASHIER_TX_TYPES.includes(t.type));
...
// поддържа ИДЕМПОТЕНТНО записа за изплатена заработка в касата, свързан с
// точно този payroll_entries запис (related_payroll_entry_id) — премахва
// стария запис (ако има) и (само ако е маркиран платен и нетната сума е
// положителна) слага нов с актуалната сума, вместо да трупа дубликати.
function syncWagePayoutKasaEntry(entry) {
  const db = readDb();
  if (!db.wallet_transactions) db.wallet_transactions = [];
  db.wallet_transactions = db.wallet_transactions.filter(
    t => !(t.type === 'wage_payout' && t.related_payroll_entry_id === entry.id)
  );
  const cashierId = db.hr_settings?.cashier_profile_id;
  const amount = Number(entry.net_amount || 0);
  if (cashierId && entry.paid && amount > 0) {
    const driver = db.profiles.find(p => p.id === entry.profile_id);
    db.wallet_transactions.push({
      id: uid('wtx'), user_id: cashierId, amount: -amount,
      type: 'wage_payout', related_payroll_entry_id: entry.id,
      note: `Изплатена заработка — ${driver ? driver.full_name : entry.profile_id} — седмица ${entry.week_start}`,
...
// еднократна миграция (безопасна за многократно пускане): преди наемът на
// кола постъпваше автоматично в касата (тип 'car_rent_income') — по изрично
// искане на потребителя това вече НЕ е касово движение, а чиста статистика
// (виж getNonCashPayrollStats), затова старите записи от тази автоматика се
// премахват, за да не изкривяват баланса на касата занапред.
function cleanupObsoleteCarRentCashierEntries() {
  const db = readDb();
  const before = (db.wallet_transactions || []).length;
  db.wallet_transactions = (db.wallet_transactions || []).filter(t => t.type !== 'car_rent_income');
  const removed = before - db.wallet_transactions.length;
  if (removed) writeDb(db);
  return removed;
}

// плащане по договор с ВЪНШЕН наемател (виж addPayment) — идемпотентно по
// related_payment_id, за да е безопасно ако някога добавим редакция на
// плащания. direction на плащането се пренася директно (income → приход в
// касата, expense → разход, напр. връщане на депозит).
function syncExternalRentCashierEntry(payment, contract) {
  const db = readDb();
  if (!db.wallet_transactions) db.wallet_transactions = [];
  db.wallet_transactions = db.wallet_transactions.filter(t => t.related_payment_id !== payment.id);
  const cashierId = db.hr_settings?.cashier_profile_id;
  const amt = Number(payment.amount || 0);
  if (cashierId && amt > 0) {
    const signedAmount = payment.direction === 'expense' ? -amt : amt;
    db.wallet_transactions.push({
      id: uid('wtx'), user_id: cashierId, amount: signedAmount,
      type: payment.direction === 'expense' ? 'external_rent_expense' : 'external_rent_income',
      related_payment_id: payment.id,
      note: [payment.description, contract ? `договор ${contract.contract_number}` : null, contract ? contract.renter_name : null].filter(Boolean).join(' · '),
      created_by: payment.created_by || null, created_at: nowIso(),
    });
  }
  writeDb(db);
}
...
// ---------------------------------------------------------------------------
// РЕМОНТ/РАЗХОД ПО КОЛА, ПЛАТЕН ОТ КАСАТА — създава едновременно (а) запис в
// сервизната книжка (service_records — вижда се в историята на колата) и
// (б) касов разход, свързан към него. Пробегът (км) НЕ е задължителен тук
// (за разлика от обичайния сервизен запис, това е бърз запис "на място").
// ---------------------------------------------------------------------------
function addCashierRepairEntry({ vehicleId, amount, description, odometerKm, serviceDate, createdBy }) {
  const db = readDb();
  const cashierId = db.hr_settings?.cashier_profile_id;
  if (!cashierId) throw new Error('Няма избран касиер — задайте го от настройките на касата.');
  const vehicle = db.vehicles.find(v => v.id === vehicleId);
  if (!vehicle) throw new Error('Колата не е намерена');
  const amt = Number(amount);
  if (!amt || amt <= 0) throw new Error('Невалидна сума');
  if (!description || !String(description).trim()) throw new Error('Нужно е описание на ремонта/разхода');

  const serviceRecord = addServiceRecord(vehicleId, {
    service_date: serviceDate || nowIso().slice(0, 10),
    type: 'repair',
    cost: amt,
    description,
    odometer_km: odometerKm != null && odometerKm !== '' ? Number(odometerKm) : undefined,
    created_by: createdBy,
    source: 'cashier',
  });

  const db2 = readDb();
  if (!db2.wallet_transactions) db2.wallet_transactions = [];
  db2.wallet_transactions.push({
    id: uid('wtx'), user_id: cashierId, amount: -amt,
    type: 'vehicle_repair', related_service_record_id: serviceRecord.id,
    note: `${vehicle.plate_number} — ${description}`,
    created_by: createdBy, created_at: nowIso(),
  });
  writeDb(db2);
  return { service_record: serviceRecord, cashier_balance: getCashierBalance(cashierId) };
}

// ---------------------------------------------------------------------------
// ДВИЖЕНИЯ ПО БАНКОВА СМЕТКА — отделен дневник от касата в брой (различни
// физически пари), но участва в общата картина за печалба за периода (виж
// getCashierPeriodSummary).
// ---------------------------------------------------------------------------
function listBankMovements({ from, to } = {}) {
  const db = readDb();
  let rows = db.bank_movements || [];
  if (from) rows = rows.filter(m => m.movement_date >= from);
  if (to) rows = rows.filter(m => m.movement_date <= to);
  return rows.sort((a, b) => (a.movement_date < b.movement_date ? 1 : -1));
}

function addBankMovement({ direction, amount, description, movement_date, created_by }) {
  if (!['income', 'expense'].includes(direction)) throw new Error('Невалиден тип движение (приход/разход)');
  const amt = Number(amount);
  if (!amt || amt <= 0) throw new Error('Невалидна сума');
  if (!description || !String(description).trim()) throw new Error('Нужно е описание на движението');
  const db = readDb();
  if (!db.bank_movements) db.bank_movements = [];
  const rec = {
    id: uid('bm'), direction, amount: amt, description,
    movement_date: movement_date || nowIso().slice(0, 10),
    created_by, created_at: nowIso(),
  };
  db.bank_movements.push(rec);
  writeDb(db);
  return rec;
}

// ---------------------------------------------------------------------------
// ОБОБЩЕНИЕ ЗА ПЕРИОД: всичко, което действително мина през касата (в брой)
// + движенията по банковата сметка — дава ясна картина каква е печалбата за
// избрания период (ден/седмица/месец — филтърът е просто from/to дати).
// ---------------------------------------------------------------------------
function getCashierPeriodSummary({ from, to } = {}) {
  const cashierId = getCashierProfileId();
  const cashTx = cashierId ? listCashierTransactions(cashierId, { from, to }) : [];
  const cashByType = {};
  cashTx.forEach(t => {
    cashByType[t.type] = (cashByType[t.type] || 0) + Number(t.amount || 0);
  });
  // депозитите (deposit_received/deposit_returned) СА реални пари, минали
  // през касата (влизат в cashTx/by_type за прозрачност на дневника), но НЕ
  // са приход/разход на фирмата — виж CASHIER_TX_TYPES_EXCLUDED_FROM_PROFIT —
  // затова се изключват тук от приход/разход/печалба и се показват отделно
  // в собствен блок "deposits" по-долу.
  const profitTx = cashTx.filter(t => !CASHIER_TX_TYPES_EXCLUDED_FROM_PROFIT.has(t.type));
  const cashIncome = profitTx.filter(t => Number(t.amount) > 0).reduce((s, t) => s + Number(t.amount), 0);
  const cashExpense = profitTx.filter(t => Number(t.amount) < 0).reduce((s, t) => s + Math.abs(Number(t.amount)), 0);

  const bankMovements = listBankMovements({ from, to });
  const bankIncome = bankMovements.filter(m => m.direction === 'income').reduce((s, m) => s + Number(m.amount), 0);
  const bankExpense = bankMovements.filter(m => m.direction === 'expense').reduce((s, m) => s + Number(m.amount), 0);

  const depositTx = cashTx.filter(t => CASHIER_TX_TYPES_EXCLUDED_FROM_PROFIT.has(t.type));
  const depositReceivedInPeriod = depositTx.filter(t => t.type === 'deposit_received').reduce((s, t) => s + Number(t.amount || 0), 0);
  const depositReturnedInPeriod = depositTx.filter(t => t.type === 'deposit_returned').reduce((s, t) => s + Math.abs(Number(t.amount || 0)), 0);

  return {
    from: from || null, to: to || null,
    cash: { income: cashIncome, expense: cashExpense, net: cashIncome - cashExpense, by_type: cashByType, transactions: cashTx },
    bank: { income: bankIncome, expense: bankExpense, net: bankIncome - bankExpense, movements: bankMovements },
    // "текущо задържани" (currently_held_total) е състояние към момента, не
    // поток за периода — затова НЕ е ограничено от from/to, виж
    // getTotalDepositsHeld по-горе.
    deposits: {
      received_in_period: Math.round(depositReceivedInPeriod * 100) / 100,
      returned_in_period: Math.round(depositReturnedInPeriod * 100) / 100,
      currently_held_total: getTotalDepositsHeld(),
      transactions: depositTx,
    },
    total_profit: (cashIncome - cashExpense) + (bankIncome - bankExpense),
  };
}
...
function markPayrollPaid(id, paid = true, markedBy = null) {
  const db = readDb();
  const rec = (db.payroll_entries || []).find(p => p.id === id);
  if (!rec) throw new Error('Записът не е намерен');
  rec.paid = paid;
  rec.paid_at = paid ? nowIso() : null;
  if (markedBy) rec.created_by = rec.created_by || markedBy;
  writeDb(db);
  // изплащането на нетната заработка е РЕАЛНО касово движение (за разлика от
  // самите удръжки/наем — виж коментара над upsertPayrollEntry)
  syncWagePayoutKasaEntry(rec);
  return rec;
}
...
// ---------------------------------------------------------------------------
// ПЛАЩАНИЯ НА ПАРТНЬОРСКИ КОМИСИОННИ — за разлика от getPartnerStats() (която
// само ИЗЧИСЛЯВА колко се дължи "на живо", без запис никъде), тук се пази
// ТРАЙНА история кога, за какъв период и колко точно е платено — и се
// синхронизира с касата (реален разход в брой), за да знае админът дали дадена
// комисионна вече е платена, за какво е и за кой период.
// ---------------------------------------------------------------------------
function listPartnerCommissionPayments({ profileId } = {}) {
  const db = readDb();
  let rows = db.partner_commission_payments || [];
  if (profileId) rows = rows.filter(p => p.profile_id === profileId);
  return rows.sort((a, b) => (a.paid_at < b.paid_at ? 1 : -1));
}

function createPartnerCommissionPayment({ profileId, periodFrom, periodTo, amount, note, createdBy }) {
  const db = readDb();
  const partner = db.profiles.find(p => p.id === profileId);
  if (!partner) throw new Error('Партньорът не е намерен');
  const amt = Number(amount);
  if (!amt || amt <= 0) throw new Error('Невалидна сума');
  if (!periodFrom || !periodTo) throw new Error('Нужен е период (от/до)');
  const cashierId = db.hr_settings?.cashier_profile_id;
  if (!cashierId) throw new Error('Няма избран касиер — задайте го от настройките на касата.');

  if (!db.partner_commission_payments) db.partner_commission_payments = [];
  const rec = {
    id: uid('pcp'), profile_id: profileId, period_from: periodFrom, period_to: periodTo,
    amount: amt, note: note || null, paid_at: nowIso(), created_by: createdBy,
  };
  db.partner_commission_payments.push(rec);

  if (!db.wallet_transactions) db.wallet_transactions = [];
  db.wallet_transactions.push({
    id: uid('wtx'), user_id: cashierId, amount: -amt,
    type: 'commission_payout', related_commission_payment_id: rec.id,
    note: `Комисионна — ${partner.full_name} — период ${periodFrom} – ${periodTo}${note ? ' · ' + note : ''}`,
    created_by: createdBy, created_at: nowIso(),
  });
  writeDb(db);
  return rec;
...
function getEmployeeDocumentAlerts() {
  ...
  (db.employment_contracts || []).forEach(c => {
    // 'draft' = чернова, която никога не е била финализирана/активирана
    // (напр. заменена веднага след това с друг тип договор) — не трябва да
    // генерира постоянна фалшива тревога "просрочен договор" за служител,
    // чийто РЕАЛЕН (активен) договор изобщо не е такъв
    if (!c.end_date || c.status === 'terminated' || c.status === 'draft') return;
...
module.exports = {
  ...
  getContractDepositState, addContractDeposit, returnContractDeposit, getTotalDepositsHeld,
  ...
  getCashierProfileId, setCashierProfileId, getCashierBalance, listCashierTransactions,
  cleanupObsoleteCarRentCashierEntries, getNonCashPayrollStats, getCashierPeriodSummary,
  addCashierRepairEntry, listBankMovements, addBankMovement,
  listPartnerCommissionPayments, createPartnerCommissionPayment,
  ...
  getSiteContent, updateSiteContent, getRentacarSiteContent, updateRentacarSiteContent,
  createRentRequest, listRentRequests, deleteRentRequest,
  ...
  listPayrollEntries, getPayrollEntry, upsertPayrollEntry, getDefaultPayrollDeductions, signPayrollEntry, markPayrollPaid,
