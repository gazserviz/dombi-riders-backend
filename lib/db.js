...
// само транзакции с тип, характерен именно за касата:
//  - 'car_rent_income'  — автоматичен наем от коли (виж syncCarRentKasaEntry)
//  - 'cashier_manual'   — ръчно въведени от super_admin движения по касата
// Лични трансфери ('transfer') и лични корекции ('admin_adjustment') на
// същия профил НЕ участват тук — те са си негов личен портфейл (wallet.html).
const CASHIER_TX_TYPES = ['car_rent_income', 'cashier_manual'];
...
function listCashierTransactions(userId) {
  const db = readDb();
  return (db.wallet_transactions || [])
    .filter(t => t.user_id === userId && CASHIER_TX_TYPES.includes(t.type))
    .sort((a, b) => (a.created_at < b.created_at ? 1 : -1));
