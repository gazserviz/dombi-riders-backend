...
function approveJobApplication(id, { reviewed_by, email, temp_password, manager_id } = {}) {
  ...
  if (app.status !== 'pending') throw new Error('Кандидатурата вече е разгледана');
...
function getActivityFeed(limit = 50) {
  ...
  db.service_records.forEach(s => {
    items.push({
      ...
      text: `Сервизен запис за ${plate(s.vehicle_id)}: ${s.description || '—'} (${Number(s.cost || 0)} лв.)`,
  ...
  db.vehicle_recurring_costs.forEach(c => {
    items.push({
      ...
      at: c.created_at,
      actor: null,
      text: `Добавен периодичен разход за ${plate(c.vehicle_id)}: ${c.type} — ${Number(c.amount || 0)} лв.`,
  ...
  db.vehicle_payments.forEach(p => {
    items.push({
      ...
      at: p.created_at,
      actor: null,
      text: `${p.direction === 'income' ? 'Приход' : 'Разход'} ${Number(p.amount || 0)} лв. за ${plate(p.vehicle_id)}`,
