...
function createFleetShowcaseItem(data) {
  ...
  const item = {
    ...
    badge: null, includes: [], requirements: [], image_url: null,
...
function getFleetStats() {
  ...
  const perVehicle = db.vehicles.map(v => {
    ...
    // реалният приход от кола идва основно от седмичните удръжки от заработката
    // на назначения шофьор (виж getCompanyFinanceReport, т. 4 — "Седмични такси
    // от шофьорите"), не само от ръчно записани vehicle_payments. Тук
    // приписваме удръжка от payroll_entries към колата, ако профилът на
    // шофьора е бил зачислен на нея през седмицата на записа — иначе
    // зачисляването само по себе си никога не мести стойностите в
    // статистиката, докато не бъде добавен ръчен запис в "Финанси".
    const payrollIncome = (db.payroll_entries || []).reduce((sum, e) => {
      if (!e.deduction_amount) return sum;
      ...
      return matches ? sum + Number(e.deduction_amount || 0) : sum;
    }, 0);
    const income = directIncome + payrollIncome;
