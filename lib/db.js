...
//  - 'external_rent_expense'— връщане на депозит/друг разход по такъв договор
...
function getCashierPeriodSummary({ from, to } = {}) {
  ...
  const cashIncome = cashTx.filter(t => Number(t.amount) > 0).reduce((s, t) => s + Number(t.amount), 0);
  const cashExpense = cashTx.filter(t => Number(t.amount) < 0).reduce((s, t) => s + Math.abs(Number(t.amount)), 0);
